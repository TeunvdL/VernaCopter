...
def run(
    ...
    p.loadURDF("cube.urdf", [2, 1, 1], useFixedBase=True)
    cube_size = [0.1, 1, 0.5]  # Define the size of the cube
    visualShapeId = p.createVisualShape(shapeType=p.GEOM_BOX,
                                        halfExtents=cube_size,
                                        rgbaColor=[0.8, 0.8, 0.8, 1])  # Create the cube's visual shape
    collisionShapeId = p.createCollisionShape(shapeType=p.GEOM_BOX,
                                              halfExtents=cube_size) # Create the cube's collision shape

    # Create the multi-body for the cube
    cubeId = p.createMultiBody(baseMass=10,
